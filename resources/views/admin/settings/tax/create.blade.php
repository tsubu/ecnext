...
@section('title', __('Establish New Regulation'))
@section('page_title', __('Create Tax Rule'))
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h3 class="text-xl font-bold tracking-tight text-white">{{ __('New Fiscal Directive') }}</h3>
        <p class="text-sm text-slate-400">{{ __('Define temporal boundaries and rate metrics for system-wide synchronization.') }}</p>
    </div>
</div>

<form action="{{ route('admin.tax_rules.store') }}" method="POST" class="max-w-4xl">
    @csrf
    ...
    <div class="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="{{ __('e.g. Standard VAT 2026') }}"
