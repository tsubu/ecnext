...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h3 class="text-xl font-bold tracking-tight text-white">{{ __('New Inventory Entry') }}</h3>
        <p class="text-sm text-slate-400">{{ __('Define core product attributes and variants for your catalog.') }}</p>
    </div>
</div>

<form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" x-data="{ tab: 'general' }">
    @csrf
            ...
            <div class="glass p-4 rounded-[2rem] border border-white/5 sticky top-24">
                ...
                <div class="mt-8 pt-8 border-t border-white/5 px-2">
                    <button type="submit" class="w-full py-4 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-2xl transition-all shadow-lg shadow-rose-600/20 active:scale-95 flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"/></svg>
                        {{ __('Complete Registry') }}
                    </button>
                </div>
            </div>
        </div>
                ...
                <div class="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="{{ __('e.g. Aura Pro Audio') }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="{{ __('e.g. aura-pro-audio') }}" required
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="short_description" id="short_description" value="{{ old('short_description') }}" placeholder="{{ __('A catchy one-liner for search results...') }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" id="description" rows="10" placeholder="{{ __('Describe the features, materials, and value...') }}" required
                            class="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-[2rem] text-slate-300 font-medium focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="sku" id="sku" value="{{ old('sku') }}" placeholder="{{ __('PRD-123-X') }}" required
                            ...
                            @error('sku') <p class="text-xs text-rose-500 font-bold ml-1">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price') }}" placeholder="{{ __('0') }}" required
                                    class="w-full pl-10 pr-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-medium focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-600">
                            </div>
                        </div>
                        <div class="space-y-2">
                    ...
                    <div class="pt-8 border-t border-white/5 space-y-6" x-data="{ 
                        ...
                        <div class="flex items-center justify-between">
                            <h4 class="text-sm font-bold text-indigo-300">{{ __('Sale Price Schedule') }}</h4>
                        ...
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest font-bold">{{ __('Optional: Define specific periods for promotional pricing.') }}</p>
                            ...
                            <div class="space-y-2 max-h-64 overflow-y-auto pr-4 custom-scrollbar">
                                @foreach($categories as $category)
                                    <label class="flex items-center gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 cursor-pointer transition-all">
                                        <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" class="w-5 h-5 rounded border-white/10 bg-white/5 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900">
            ...
            <div x-show="tab === 'media'" x-cloak class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div class="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
                    ...
                    <div class="border-2 border-dashed border-white/10 rounded-[2rem] p-12 text-center hover:border-indigo-500/50 transition-all group">
                        <input type="file" name="images[]" id="images" multiple class="hidden" @change="/* Simple preview handled by browser */">
                        <label for="images" class="cursor-pointer flex flex-col items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm font-bold text-slate-300">{{ __('Drop imagery here or click to browse') }}</p>
            ...
            <div x-show="tab === 'seo'" x-cloak class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div class="glass p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title') }}" placeholder="{{ __('Recommended: Name | Brand') }}"
                            class="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-medium focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta_description" id="meta_description" rows="4" placeholder="{{ __('Optimal: 155-160 characters describing your product value.') }}"
                            class="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-slate-300 font-medium focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all">{{ old('meta_description') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
