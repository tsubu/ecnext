...
@section('page_title', __('Forge Promotional Event'))
...
<div class="max-w-5xl mx-auto">
    ...
    <form action="{{ route('admin.campaigns.store') }}" method="POST" class="space-y-8">
        @csrf
        
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 text-glow-indigo">
            <div class="lg:col-span-2 space-y-8">
                <div class="glass p-10 rounded-[2.5rem] border border-white/5 space-y-10 shadow-2xl">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. 2026 Winter Mega Sale" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-black text-xl tracking-tight focus:ring-2 focus:ring-rose-500/50 outline-none transition-all placeholder:text-white/10">
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="description" rows="4" placeholder="Mention the terms of the sale..." class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-medium focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all placeholder:text-white/10">{{ old('description') }}</textarea>
                    </div>

                    <!-- Discount Logic -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-rose-500/5 rounded-3xl border border-rose-500/10">
                        <div class="space-y-3">
                            ...
                            <select name="discount_type" required class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-bold outline-none focus:border-rose-500">
                                <option value="percentage" {{ old('discount_type') == 'percentage' ? 'selected' : '' }}>{{ __('Percentage Reduction (%)') }}</option>
                                <option value="fixed" {{ old('discount_type') == 'fixed' ? 'selected' : '' }}>{{ __('Fixed Amount Off (¥)') }}</option>
                            </select>
                        </div>
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-rose-400 uppercase tracking-[0.2em] ml-1">{{ __('Reduction Value') }}</label>
                            <input type="number" name="discount_value" value="{{ old('discount_value') }}" required placeholder="0" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-rose-500 text-lg font-black outline-none focus:border-rose-500">
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="max-h-64 overflow-y-auto pr-4 space-y-2 custom-scrollbar">
                            @foreach($products as $product)
                                <label class="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 cursor-pointer group transition-all">
                                    <input type="checkbox" name="products[]" value="{{ $product->id }}" class="peer hidden">
                                    <div class="w-5 h-5 bg-white/10 border border-white/10 rounded-lg group-hover:bg-white/20 peer-checked:bg-rose-600 peer-checked:border-rose-500 transition-all flex items-center justify-center">
                        ...
                        <p class="text-[9px] text-slate-600 font-bold italic">{{ __('Only active products are listed here.') }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="glass p-8 rounded-[2.5rem] border border-white/5 space-y-8">
                    <div class="space-y-3">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="datetime-local" name="starts_at" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-xs outline-none focus:ring-1 focus:ring-rose-500">
                            </div>
                            <div>
                                <label class="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-2 block">{{ __('Ends At') }}</label>
                                <input type="datetime-local" name="expires_at" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-xs outline-none focus:ring-1 focus:ring-rose-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 px-1">
                        <div class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div class="w-11 h-6 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-rose-600 transition-all"></div>
                        </div>
                        <span class="text-xs font-bold text-slate-400 select-none">{{ __('Activate Campaign') }}</span>
                    </div>
                </div>

                <div class="p-8 bg-rose-600/10 border border-rose-500/20 rounded-[2.5rem] space-y-4">
                    <p class="text-[10px] text-rose-400 font-black uppercase tracking-widest">{{ __('Live Preview Hint') }}</p>
                    <p class="text-xs text-rose-300/60 leading-relaxed font-medium">
                        {{ __('Once active, target products will display the original price struck-through with this promotional price highlighted.') }}
                    </p>
                </div>
