...
<div x-data="blockEditor()" class="max-w-[1600px] mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div class="lg:col-span-5 space-y-8">
            <form action="{{ route('admin.block-instances.store') }}" method="POST" id="block-form" class="space-y-8">
                @csrf
                <div class="glass p-8 rounded-[2.5rem] border border-white/5 space-y-8 shadow-2xl">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white text-sm outline-none focus:border-indigo-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">{{ __('Slug') }}</label>
                            <input type="text" name="slug" value="{{ old('slug') }}" class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-indigo-300 font-mono text-xs outline-none focus:border-indigo-500">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                            @foreach(block_admin_locale_rows() as $row)
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="name_locales[{{ $row['code'] }}]" value="{{ old('name_locales.'.$row['code']) }}" class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white text-sm outline-none focus:border-indigo-500" placeholder="{{ $row['placeholder'] }}">
                                </div>
                            @endforeach
                        </div>
                        <div class="space-y-2">
            ...
            <div class="sticky top-8 h-[calc(100vh-8rem)] min-h-[20rem]" x-bind:class="{ 'hidden': mode !== 'visual' }">
                <div class="h-full glass rounded-[3rem] border border-white/5 overflow-hidden flex flex-col shadow-2xl">
                    ...
                    <div class="flex-grow bg-[#fbfbfb] relative overflow-auto min-h-0">
                        ...
                        <div x-show="!previewHtml && !loading" class="absolute inset-0 flex flex-col items-center justify-center p-20 text-center">
                            ...
                            <h3 class="text-slate-400 font-bold uppercase tracking-widest text-xs">{{ __('Awaiting Configuration') }}</h3>
                            <p class="text-slate-300 text-[10px] mt-2 italic">{{ __('Select a template and configure settings to see results here.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
function blockEditor() {
    return {
        mode: 'visual',
        blockTypeId: '{{ old('block_type_id', request('type_id')) }}',
        settings: {},
        settingsJson: '{}',
        settingsLocalesJson: {!! json_encode(old('settings_locales') ?? '{}') !!},
        ...
        onTypeChange() {
            ...
            if (selectedOption && selectedOption.dataset.schema) {
                ...
                // Initialize settings with empty schema values if not already set
                let newSettings = {};
