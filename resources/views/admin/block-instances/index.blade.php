...
@section('title', __('Block Instances'))
@section('page_title', __('Reusable Content Components'))

@section('content')
<div class="mb-8 flex justify-between items-end">
    <div>
        <h3 class="text-xl font-bold tracking-tight text-white">{{ __('Component Library') }}</h3>
        <p class="text-sm text-slate-400 mt-1">{{ __('Create concrete content for your block types to use across multiple pages.') }}</p>
    </div>
    <div class="flex gap-3">
        <a href="{{ route('admin.block-types.index') }}" class="px-6 py-3 bg-white/5 hover:bg-white/10 text-white text-sm font-bold rounded-2xl transition-all border border-white/5 active:scale-95">
            {{ __('Manage Types') }}
        </a>
        <a href="{{ route('admin.block-instances.create') }}" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-2xl transition-all shadow-lg shadow-indigo-600/20 flex items-center gap-2 active:scale-95">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-white/5 bg-white/5">
                    <th class="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{{ __('Instance Name') }}</th>
            ...
            <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-indigo-400 border border-white/5 group-hover:border-indigo-500/30 transition-all">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{{ $instance->resolvedName() }}</span>
                                    <span class="text-[10px] text-slate-500 mt-0.5 tracking-wider font-medium">{{ __('Modified') }} {{ $instance->updated_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <span class="px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                {{ $instance->blockType->resolvedName() }}
                            </span>
                        </td>
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-all scale-95 group-hover:scale-100">
                                <a href="{{ route('admin.block-instances.edit', $instance) }}" class="p-2 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white rounded-xl transition-all border border-white/5">
                                ...
                                <form action="{{ route('admin.block-instances.destroy', $instance) }}" method="POST" onsubmit="return confirm('{{ __('Delete this block instance?') }}')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-8 py-20 text-center text-slate-500 font-medium italic">
                            {{ __('No block instances created yet.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
