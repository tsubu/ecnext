...
@section('title', __('edit') . ': ' . $notice->title)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h3 class="text-xl font-bold tracking-tight text-white">{{ __('Modify Announcement') }}</h3>
        <p class="text-sm text-slate-400">{{ __('Update the details and schedule of this storefront news entry.') }}</p>
    </div>
</div>
            ...
            <div class="glass p-8 rounded-[2rem] shadow-2xl border border-white/5 space-y-6">
                ...
                <div class="space-y-2 pt-2">
                    ...
                    <textarea name="content" id="content" rows="12" required
                        class="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-3xl text-slate-200 font-medium focus:ring-2 focus:ring-rose-500/50 focus:border-rose-500 outline-none transition-all">{{ old('content', $notice->content) }}</textarea>
        ...
        <div class="lg:col-span-4 space-y-8">
            <div class="glass p-8 rounded-[2rem] shadow-2xl border border-white/5 space-y-6 sticky top-24">
                ...
                <div class="space-y-2">
                    ...
                    <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $notice->published_at ? $notice->published_at->format('Y-m-d\TH:i') : '') }}"
                    ...
                    <p class="text-[10px] text-slate-500 font-medium uppercase tracking-wider ml-1 mt-1">{{ __('When this news becomes visible.') }}</p>
                </div>

                <div class="space-y-2 pt-2">
                    <label for="expired_at" class="text-sm font-bold text-slate-300 ml-1">{{ __('Schedule End') }}</label>
                    <input type="datetime-local" name="expired_at" id="expired_at" value="{{ old('expired_at', $notice->expired_at ? $notice->expired_at->format('Y-m-d\TH:i') : '') }}"
                        class="w-full px-5 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:ring-2 focus:ring-rose-500/50 focus:border-rose-500 outline-none transition-all">
                ...
                <div class="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 mt-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $notice->is_active) ? 'checked' : '' }}>
